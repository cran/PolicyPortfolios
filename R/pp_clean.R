...
#' @param setting.direction Logical. When TRUE, returns not a tidy object with the portfolio, but a summary of changes in the Setting (Level/Scope) and in the Direction (Expansion/Dismantling). Defaults to FALSE.
...
pp_clean <- function(d, Sector = NULL,
  ...
  if (!is.null(associated.vars)) {
    if (inherits(associated.vars, "character")) {
  ...
  if (length(which(is.na(D.changes$Instrument[D.changes$cc == coding.category]))) > 0) {
    ...
    # Add 1 to match the real row in the spreadsheet, as it contains a header
    message(paste("Row: ", 1 + which(is.na(D.changes$Instrument & D.changes$cc == coding.category)), "\n", sep = ""))
  }
  if (length(which(is.na(D.changes$Target[D.changes$cc == coding.category]))) > 0) {
    ...
    # Add 1 to match the real row in the spreadsheet, as it contains a header
    message(paste("Row: ", 1 + which(is.na(D.changes$Target& D.changes$cc == coding.category)), "\n", sep = ""))
  }
...
#                       covered = 0) %>% # Assumption
    ...
    for (o in seq_len(dim(D.changes)[1])) {
      ...
      if (length(value.now) > 0) {
        if (is.na(value.now) & direction.now == 1) {
          ...
          if (debug) {
            message("\n\n=== Debug case ===")
            message(paste("* There is a status quo entry in a space that apparently was not being covered\nby policy intervention before at \n(Country, Instrument, Target, Year) ", D.changes$Country[o], D.changes$Instrument[o], D.changes$Target[o], D.changes$Year[o], sep = "\n "))
          ...
          if (debug ) {
            message("\n\n=== Debug case ===")
            message(paste("* There is a dismantling entry in a space that apparently\nwas not being covered by policy intervention before at\n(Country, Instrument, Target, Year) ", D.changes$Country[o], D.changes$Instrument[o], D.changes$Target[o], D.changes$Year[o], sep = "\n "))
          ...
          if (debug) {
            message("\n\n=== Debug case ===")
            message(paste("* There is a policy change from no intervention to status quo at\n(Country Instrument Target Year) ", D.changes$Country[o], D.changes$Instrument[o], D.changes$Target[o], D.changes$Year[o], sep = "\n "))
          ...
          if (debug) {
            ...
            message("This can happen BUT only assuming that the policy was already in place at\n(Country Instrument Target Year), at the beginning of the time period and\nthat this is the first changein this space.")
            message("Proceeding with this assumption.")
          }
    ...
    D <- D.changes %>%
      dplyr::as_tibble() %>%
      dplyr::rename(`Coding category` = cc) %>%
      dplyr::rename(Direction = direction) %>%
      dplyr::filter(`Coding category` %in% c(3, 4)) %>%
      dplyr::filter(Direction != 0) %>%
      dplyr::count(Country, Year, `Coding category`, Direction) %>%
      dplyr::mutate(Direction = case_when(
        Direction == 1 ~ "Expansion",
        Direction == -1 ~ "Dismantling",
        TRUE ~ NA_character_)) %>%
      dplyr::mutate(Setting = case_when(
        `Coding category` == 3 ~ "Level",
        `Coding category` == 4 ~ "Scope")) %>%
      dplyr::select(-`Coding category`) %>%
      dplyr::rename(nChanges = n) %>%
      dplyr::arrange(Country, Year, Setting, Direction) %>%
      dplyr::select(Country, Year, Setting, Direction, nChanges)
    return(D)
  }
}
